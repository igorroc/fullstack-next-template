...
import { MultipleSelect } from "@/components/profile/teste"
...
export function ProfileContent({ user }: ProfileContentProps) {
    ...
    return (
        <main className="relative flex min-h-dvh flex-col items-center overflow-hidden px-6 py-12">
            <div
                className="absolute left-1/2 top-10 h-72 w-72 -translate-x-1/2 rounded-full bg-emerald-400/15 blur-3xl"/>
            <div className="w-full max-w-3xl space-y-6">
                <Card
                    className="w-full border border-white/15 bg-white/10 shadow-2xl shadow-emerald-950/30 backdrop-blur-xl">
					<Card.Header className="flex flex-col items-center gap-4 pb-4 pt-8 text-center">
                        <Chip
                            color="success"
							variant="soft"
                            size="sm"
					...
					<Card.Content className="gap-6 px-6 pb-8">
						<div className="flex flex-col items-center gap-4">
							<MultipleSelect />
							<Avatar size="lg" className="h-24 w-24 bg-gradient-to-br from-emerald-300 to-sky-400 text-large text-slate-950">
								<Avatar.Fallback>
